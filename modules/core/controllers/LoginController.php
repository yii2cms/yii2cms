...
class LoginController extends \app\modules\core\classes\FrontController
{
    ...
    public function actionIndex()
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if (!$captcha_code) {
                Yii::$app->session->setFlash('error', '图形验证码不能为空');
            ...
            if (!$captchaAction->validate($captcha_code, false)) {
                Yii::$app->session->setFlash('error', '图形验证码错误');
                return $this->render('index', $data);
            }
            ...
            if ($login_type == 'account') {
                ...
                if ($user && $user->validatePassword($password)) {
                    if ($user->status == 'disabled') {
                        Yii::$app->session->setFlash('error', '用户已被禁用');
                        return $this->render('index', $data);
                    }
                    ...
                    Yii::$app->session->setFlash('error', '邮箱或密码错误');
                }
                ...
                if ($user) {
                    ...
                    if (!is_local() &&  (!$code || $code != $email_code)) {
                        Yii::$app->session->setFlash('error', '邮箱验证码错误');
                    } else {
                        if ($user->status == 'disabled') {
                            Yii::$app->session->setFlash('error', '用户已被禁用');
                            return $this->render('index', $data);
                    ...
                    Yii::$app->session->setFlash('error', '邮箱不存在');
                }
                ...
                if ($user) {
                    ...
                    if (!is_local() && (!$code || $code != $phone_code)) {
                        Yii::$app->session->setFlash('error', '手机号验证码错误');
                    } else {
                        if ($user->status == 'disabled') {
                            Yii::$app->session->setFlash('error', '用户已被禁用');
                            return $this->render('index', $data);
                    ...
                    Yii::$app->session->setFlash('error', '手机号不存在');
                }
            }
        }
